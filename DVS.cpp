...
void Candidate::UpdateData()
{
	...
	if (CheckRecord(filename, CandidateID))
	{
		...
		colour("\n\t\tCandidate Name: ", 14);
		cout << CandidateRecord[1];
		colour("\n\t\tParty Name: ", 14);
		cout << CandidateRecord[2];
		colour("\n\t\tParty Symbol: ", 14);
		cout << CandidateRecord[3];
		int Index;
		...
		case 1:
		{
			colour("\tCurrent Candidate Name: ", 14);
			cout << CandidateRecord[1];
			cin.ignore();
		...
		case 3:
		{
			colour("\tCurrent Party Symbol: ", 14);
			cout << CandidateRecord[3];
			cin.ignore();
...
void Poll::AddData()
{
	...
	if (!CheckRecord(filename, PollID))
	{
		...
		file << PollID << "," << PollName << "," << PollDate << ","
			 << "Upcoming" << endl;
		file.close();

		fstream PollFile;
		PollFile.open(PollFileName, ios::out | ios::app);
		PollFile << "CandidateID"
				 << ","
				 << "Votes" << endl;
		PollFile.close();
		bool Add = true;
